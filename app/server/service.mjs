...
async function getRecords(Model, req, res) {
    try {
        ...
        if (records.length > 0) {
            ...
            res.status(204).json({message: 'no records'})
        }
    } catch (error) {
...
async function getEvents(Event, req, res) {
    try {
...
async function postEvent(Event, req, res) {
    try {
        if (valid(Event, req.body)) {
            let record = await Event.create(req.body)
            res.status(201)
                .location(`http://${req.headers.host}${req.baseUrl}${req.url}${req.url.endsWith('/') ? '' : '/'}${req.params.groupId}/${record.id}`)
                .send()
            record.save()
        } else {
            res.status(400).send()
